...
export function QuickSettingsDropdown() {
  ...
  const getModelDisplayName = (modelName: string) => {
    ...
    return model?.displayName || modelName.replace('gemini-', 'Gemini ');
  };
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute right-0 top-full mt-2 w-80 bg-gray-900 border border-gray-700 rounded-xl shadow-2xl z-[60] overflow-hidden"
          >
                ...
                >
                  ...
                  {models.length > 0 && (
                    <optgroup label="All Models">
                      {models.filter(m => !quickModels.includes(m.name)).map(model => (
